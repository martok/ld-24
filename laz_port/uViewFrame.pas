...
uses
  ...
  Controls, Forms, Contnrs, Dialogs, dglOpenGL, TextSuite, ExtCtrls,
  OpenGLContext, uCity, FastGL;
...
begin
  ...
  guiMain.ClientRect := Classes.Rect(ClientWidth - GUI_WIDTH, 0, ClientWidth, ClientHeight);
  PushLayer(guiMain);
...
begin
  ...
  glEnable(GL_BLEND);
  if Assigned(City) then begin
    r:= classes.rect(-1,0,ClientWidth-GUI_WIDTH, 40);
...
begin
  ...
  if (clr[2] > 0) then
    result := Classes.Point(clr[0], clr[1])
  else
    result := Classes.Point(-1, -1);
end;
  ...
  if not HasMoved then begin
    if not ((GUIStack.Count > 0) and TGUILayer(GUIStack[GUIStack.Count-1]).MouseClick(MP.X, MP.Y)) and
      ...
      if (p.X >= 0) and (p.Y >= 0) then begin
        ...
        block.ClientRect := Classes.Rect(ClientWidth - GUI_WIDTH, 0, ClientWidth, ClientHeight);
        PushLayer(block);
      end;
    end;
...
var
  i: integer;
begin
  result := PtInRect(ClientRect, Classes.Point(X, Y));
  if result then begin
    dec(X, ClientRect.Left);
    dec(Y, ClientRect.Top);
    for i:= 0 to fClickables.Count - 1 do begin
      if PtInRect(TGUIClickable(fClickables[i]).ActiveRect, Classes.Point(X, Y)) then begin
...
begin
  fMousePos := Classes.Point(X - ClientRect.Left, Y - ClientRect.Top);
end;
