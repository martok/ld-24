...
begin
  ...
  for Idx := Low(fHashArray) to High(fHashArray) do begin
    ...
    while Entry <> nil do begin
      NameList.Add(Pointer(Entry^.Name));
...
begin
  if pString <> nil then begin
    Pos := Cardinal(pString) mod fHashEntrys;
    Entry := fHashArray[Pos];
    HashEntry := Entry;
...
begin
  // Needs to use an widestring because of #0 endchar
  Temp := CharCode;
    ...
    if not (fFlags and TS_BLOCKFLAG_NO_CLIP = TS_BLOCKFLAG_NO_CLIP) then begin
      case tsGetParameteri(TS_CLIP) of
        TS_CLIP_COMPLETE: begin
          if (fLineTop + fTextOffsetY < fBlockTop) or ((fLineTop + fTextOffsetY - LineSkip) > (fBlockTop + fBlockHeight)) then
            DrawText := False; 
        end;
        TS_CLIP_BORDER: begin
          if ((fLineTop + fTextOffsetY - LineSkip) < fBlockTop) or (fLineTop + fTextOffsetY > (fBlockTop + fBlockHeight)) then
            DrawText := False; 
        end;
      end;
    end;
